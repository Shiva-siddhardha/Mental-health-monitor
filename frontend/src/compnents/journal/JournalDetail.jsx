...
const JournalDetail = () => {
    ...
    useEffect(() => {
        const fetchJournal = async () => {
            try {
                setLoading(true);
                console.log(`Fetching journal data from: http://localhost:5000/${username}/${id}`);
                const response = await axios.get(`http://localhost:5000/${username}/${id}`);
                console.log('Journal data received:', response.data);
                setJournal(response.data);
                
                // Analyze mood once journal data is loaded
                if (response.data && response.data.article) {
                    const analyzedMood = analyzeMood(response.data.article);
                    console.log('Mood analysis result:', analyzedMood);
    ...
    const analyzeMood = (text) => {
        if (!text || typeof text !== 'string') {
            console.error('Invalid text input for mood analysis:', text);
            return { type: 'neutral', score: 0, emoji: '😐' };
        }
        ...
        console.log('Starting mood analysis for text of length:', text.length);
        ...
        Object.keys(moodKeywords).forEach(moodType => {
            ...
            moodKeywords[moodType].forEach(keyword => {
                ...
                if (matches) {
                    ...
                    console.log(`Found ${matches.length} occurrences of "${keyword}" (${moodType})`);
                }
            });
        });
        
        console.log('Mood counts:', moodCounts);
        console.log('Total mood words:', totalMoodWords);
        ...
        console.log('Dominant mood:', dominantMood, 'with count:', highestCount);
        ...
        if (totalMoodWords > 0) {
            ...
            console.log('Text length:', textLength, 'Mood density:', moodDensity, 'Mood score:', moodScore);
        }
    ...
    if (loading || !journal) {
        return (
            <>
                <Navbar />
                <div className="bg-white py-16 px-6 text-center">Loading journal data...</div>
            </>
        );
    }
